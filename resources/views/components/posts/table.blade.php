@if (Session::has('success'))
    <div x-data="{ toastAlert: true }" x-show="toastAlert" x-transition @click.outside="toastAlert = false" id="toast-success"
        ...
        <button @click="toastAlert = false" type="button"
        ...
        <div class="bg-white dark:bg-gray-800 relative border sm:rounded-lg">
            <div class="flex flex-col md:flex-row items-center justify-start space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <div
                    ...
                    <a href="/dashboard/create"
                        class="flex items-center justify-center text-white bg-blue-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800 cursor-pointer">
            ...
            <div class="">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 relative">
                    ...
                    <tbody>
                        @forelse ($posts as $post)
                            <tr class="border-b dark:border-gray-700">
                                <th scope="row"
                                    ...
                                    {{ $posts->firstItem() + $loop->index }}</th>
                                <td class="px-4 py-3 hidden lg:block"><img class="w-40"
                                ...
                                <td class="px-4 py-3 flex items-center justify-end relative" x-data="{ open: false }">
                                    <button @click=" open = !open " x-bind:class="open ? 'bg-gray-200' : 'bg-none'"
                                        class="inline-flex items-center p-1 rounded-sm text-sm font-medium text-center text-gray-500 hover:text-gray-800 focus:outline-none dark:text-gray-400 dark:hover:text-gray-100 hover:bg-gray-200"
                                        type="button">
                                    ...
                                    <div x-show="open" @click.outside="open = false"
                                        class="absolute -left-45 z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                                ...
                                                <div class="relative p-4 w-full max-w-md">
                                                    ...
                                                    <div @click.outside="openDeleteModal = false"
                                                        ...
                                                        <p class="mb-4 text-gray-500 dark:text-gray-300">
                                                            Apakah anda yakin ingin menghapus permanen postingan artikel
                                                            ini
                                                        </p>
                                                        ...
                                                        <div class="flex justify-center items-center space-x-4">
                                                            ...
                                                            <form action="/dashboard/{{ $post->slug }}"
                                                                method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit"
                        ...
                        @empty
                            <tr class="border-b dark:border-gray-700">
                                <td colspan="5" class="px-4 py-3"><span class="text-sm">tidak ada data yang
                                        ditemukan. <a class="text-blue-500 hover:underline"
                                            href="/dashboard">reload</a> untuk memuat ulang</span></td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if ($posts->hasPages())
                <div class="full flex justify-center">
                    <x-posts-pagination :posts="$posts"></x-posts-pagination>
                </div>
            @endif
        </div>
    </div>
